...
async function renderSlide(text, bgImage) {
  ...
  // 2. Portrait box — bottom-right, 3:4 proportions
  const PW = 300;
  const PH = 320;
  const PX = W - PW - 48;
  const PY = H - PH - 55;
  ...
  // 3. Dialogue text — top area, stops above portrait
  const MARGIN = 68;
  const TEXT_MAX_W = W - MARGIN * 2;
  const TEXT_START_Y = 90;
  const TEXT_END_Y = PY - 48;
  ...
  ctx.fillText('— Madame Miriam —', W / 2, H - 22);
...
app.post('/render', async (req, res) => {
  try {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const url = await uploadToImgbb(buf.toString('base64'), imgbbKey);
      console.log('Slide', i + 1, 'done:', url);
      imageUrls.push(url);
    }
...
app.get('/', (req, res) => res.json({ status: 'ok', service: 'Madame Miriam Canvas Renderer v4', canvas: W + 'x' + H }));
